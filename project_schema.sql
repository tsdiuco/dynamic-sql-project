...
DROP TABLE IF EXISTS
   UserReview,
   IngredientsListAddOns,
   IngredientsListSpirits,
   CocktailRecipies,
   AddOnsUserHas,
   SpiritsUserHas,
   AppUser,
...
CREATE TABLE AddOnsUserHas(
   username    VARCHAR(20),
   add_on      VARCHAR(20),
   PRIMARY KEY (username, add_on),
   FOREIGN KEY (username)  REFERENCES AppUser(username),
   FOREIGN KEY (add_on)    REFERENCES AddOns(add_on_name)
);
...
CREATE TABLE CocktailRecipies(
   ...
   FOREIGN KEY (main_add_on) REFERENCES AddOns(add_on_name)
);

CREATE TABLE IngredientsListSpirits(
   cocktail_name   VARCHAR(20),
   spirit         VARCHAR(20),
   amount         VARCHAR(20) NOT NULL,
   PRIMARY KEY (cocktail_name, spirit),
   FOREIGN KEY (cocktail_name) REFERENCES CocktailRecipies(cocktail_name),
   FOREIGN KEY (spirit) REFERENCES Spirit(spirit_name)
);

CREATE TABLE IngredientsListAddOns(
   cocktail_name  VARCHAR(20),
   add_on            VARCHAR(20),
   amount            VARCHAR(20),
...
CREATE TABLE UserReview(
   ...
   FOREIGN KEY (username) REFERENCES AppUser(username),
   FOREIGN KEY (cocktail) REFERENCES CocktailRecipies(cocktail_name),
...
INSERT INTO SpiritsUserHas VALUES
   ('tsdiuco', 'Dark Rum'),
   ('tsdiuco', 'Light Rum'),
...
INSERT INTO AddOnsUserHas VALUES
   ('tsdiuco', 'Lime Juice'),
...
INSERT INTO CocktailRecipies VALUES
   ('Strawberry Daiquiri', 31,   'Light Rum',   'Strawberries'),
...
INSERT INTO IngredientsListSpirits VALUES
   ('Strawberry Daiquiri', 'Light Rum',   '2 shots'),
...
INSERT INTO IngredientsListAddOns VALUES
   ('Strawberry Daiquiri',    'Strawberries',            '5 crushed'),
...
INSERT INTO UserReview VALUES
   ('tsdiuco',    'Dark and Stormy',      "2021-07-31", 5),
